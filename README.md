...
CPBC Catholic Peace Broadcasting의 효율적인 차량 관리와 배차를 위한 웹 기반 시스템입니다.
...
### 5. 데이터 관리 시스템
- **파일 기반 저장**: JSON 파일로 데이터 영구 저장
- **자동 백업**: localStorage를 통한 자동 백업
- **데이터 내보내기/가져오기**: JSON 파일로 데이터 백업 및 복원
- **백업 및 복원**: 일일 백업 생성 및 특정 날짜 복원
...
- **아이콘**: Font Awesome
- **데이터 저장**: JSON 파일 + localStorage 백업
